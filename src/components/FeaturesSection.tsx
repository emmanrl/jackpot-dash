      ...
      <div className="container mx-auto max-w-7xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {features.map((feature, index) => (
            <Card
              key={index}
              className={`p-4 sm:p-6 hover:shadow-xl transition-all duration-300 hover:scale-105 opacity-0 animate-fade-in bg-gradient-to-br ${feature.gradient} border-2 hover:border-primary/30`}
              style={{ animationDelay: `${index * 0.15}s` }}
            >
              <div className="bg-background w-12 h-12 sm:w-14 sm:h-14 rounded-full flex items-center justify-center mb-3 sm:mb-4 shadow-md">
                <feature.icon className="w-6 h-6 sm:w-7 sm:h-7 text-primary" />
              </div>
              <h3 className="text-base sm:text-lg md:text-xl font-bold mb-2 text-foreground">{feature.title}</h3>
